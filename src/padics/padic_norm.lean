...
then (multiplicity (p : ℤ) q.num).get
  ...
  (multiplicity (p : ℤ) q.denom).get
    (multiplicity.finite_int_iff.2 ⟨h.2, by norm_cast_a using rat.denom_ne_zero _⟩)
else 0
...
lemma padic_val_rat_def (p : ℕ) [hp : p.prime] {q : ℚ} (hq : q ≠ 0) : padic_val_rat p q =
  ...
  (multiplicity (p : ℤ) q.denom).get (finite_int_iff.2 ⟨hp.ne_one, by norm_cast_a using rat.denom_ne_zero _⟩) :=
...
have q*r = (q.num * r.num) /. (↑q.denom * ↑r.denom),
  by {rw rat.mul_num_denom, norm_cast1},
...
have hqd : (q.denom : ℤ) ≠ 0, by norm_cast_a using rat.denom_ne_zero _,
have hrn : r.num ≠ 0, from rat.num_ne_zero_of_ne_zero hr,
have hrd : (r.denom : ℤ) ≠ 0, by norm_cast_a using rat.denom_ne_zero _,
...
have hp' : (↑p : ℚ) ≥ 1, from show ↑p ≥ ↑(1 : ℕ), by norm_cast_a using (le_of_lt hp.gt_one),
...
begin
  ...
  { apply fpow_le_of_le hp',
    ...
    { norm_cast1,
      ...
      norm_cast_a using hd },
    { norm_cast_a using hz }
  }
end
